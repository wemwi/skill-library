# City — neue Stadt onboarden (manuelles Runbook)

**Kein Agent — ein manuelles Runbook.** Eine neue Stadt live zu schalten ist bewusst ein menschlicher Schritt: Channel anlegen, Pinned setzen, Launch-Post absetzen, `liftr_city`-Eintrag pflegen. Der `pos-store`-Agent läuft für den **ersten** Store einer noch nicht existierenden Stadt bewusst **fail-closed** (`store.md` §5.1) — er legt keine Stadt an. Diese reference ist das Handbuch für den Menschen, der das dann tut. Sie wird von **keinem** laufenden Agenten geladen.

Diese Datei trägt das **Channel-Lifecycle**-Wissen (in 5.8.0 aus dem aufgelösten `telegram.md` konsolidiert). Das **Post-Format** für laufende Broadcasts liegt nicht hier, sondern bei der erzeugenden Domäne (📦/🌿 → `restock.md`, 🎉 → `store.md`) und folgt der Grammatik aus `SKILL.md` Invariante 6.
...
Die **Stadt selbst** kommt nie aus einer Belegadresse, sondern aus dem Store-Metaobjekt (redaktionelle Zuordnung — ein Store in Wunstorf kann dem Hannover-Channel zugeordnet sein). Der Channel selbst (City → `chat_id`/Username) ist ein direkter Lookup in `registry.md` §1 — **jede neu live geschaltete Stadt braucht dort einen Eintrag**, sonst findet `restock.md` ihren Channel nicht.

---

## 3. Pinned-Post (kanonisch = Live-Stand)
...
### 3.1 Pin-Reihenfolge (verifiziert)
...
### 3.2 Attributions-Poll (Launch-Element)
...
## 4. Launch-Strategie (Channel-Launch)

Betrifft den **Channel-Start einer Stadt** — nicht das Anlegen eines neuen POS-Partners (das ist die Store-Domäne, `store.md`).
...
- **Launch-Ausnahme:** „JETZT LIVE" / „in Echtzeit" sind hier okay. Hart bleibt: **keine Wirkungsangaben**, kein Link.
...
## 6. Privater Rückkanal (DM) — Restock-Input
