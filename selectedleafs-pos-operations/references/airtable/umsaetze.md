...
- **`Provision`** (Formel, €) — **der Geldwert dieser Tabelle:** `(Nettoumsatz × ⚙ Provision) × (1 + ⚙ MwSt.)`, brutto. Rollt zu `Vertriebler.Provision`. Wirksam **bei Zahlung**.
...
- **`⚙ Realprovision`** (Formel, €) — `(Bezahlt × ⚙ Provision) × (1 + ⚙ MwSt.)` — Provision auf den **tatsächlich gezahlten** Betrag (bei Teilzahlung < Provision). Rollt zu `Vertriebler.⚙ Realprovision`.
...
- **`⚙ Provision`** / **`⚙ MwSt.`** (Lookup %, ← Konditionen) · **`⚙ Fällig am`** (Datum) · `Datum` (Lookup ← Beleg, EU) · `⚙ Nettoumsatzerlös` (Rollup) · `⚙ Hinweis`.
...
| ⚙ Provision | lookup % (← Konditionen) | `fld7cBwzN5gapHOsi` |
