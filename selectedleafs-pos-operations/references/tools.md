...
- **⚠️ OFFEN: wie liest der Aufrufer die Rückgabe des Kindes?** `{{<id>.<name>}}`, `{{<id>.output.<name>}}`, `{{<id>.data.<name>}}` und `{{<id>.result.<name>}}` lösen **alle nicht** auf — auch nicht, wenn man das `metadata.interface` des Aufrufmoduls von Hand mit den Kind-Outputs bestückt. Vermutung: die Referenz entsteht erst, wenn das Aufrufmodul **einmal im Make-Designer** konfiguriert wurde (dort zieht Make das Interface aus dem Kind). **Vor dem ersten Resolver-Aufruf aus einem Worker klären** — bis dahin ist der schreibende Teil der Migration blockiert. Messmethode ohne Modul-Ausgaben: Sonden-Module hinter Filtern setzen und die **Operations-Zahl** vergleichen (`code:ExecuteCode` zählt 1, `Start`/`Router`/`Merge`/`IfElse`/`CallSubscenario` zählen 0).
