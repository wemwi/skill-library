...
- **Array-Inputs an ein Subszenario funktionieren** (verifiziert 2026-08-21): ein ganzes Array lässt sich als `data.<name>` mappen (`{{2.result.lines}}`), die Item-Schlüssel müssen zur Spec passen. Auch **`Wait for the scenario to finish` = true** läuft; der Kind-Lauf erscheint in `executions_list` mit `parent`-Block.
- **Rückgabe des Kindes lesen — `{{<modul-id>.<output-name>}}`** (verifiziert 2026-08-21, am laufenden Resolver gemessen). Damit das trägt, müssen **drei** Dinge stimmen:
- **Der Blueprint-Import leert das Interface — auch wenn es in der Datei steht.** Am 2026-08-21 an `[Process] Upload PDF (Inventory)` live passiert: nach dem Import stand `interface.input` auf `[]`, während das Szenario **aktiv** war. Der Dispatcher hätte seine drei Eingaben nicht mehr übergeben können. **Pflichtschritt nach jedem Import: `scenarios_interface` lesen und bei Bedarf mit `scenarios_set-interface` wiederherstellen** — vor dem nächsten produktiven Lauf. Beim Wiederherstellen: **leere `help`-Strings weglassen**, die API weist `help: ""` mit „should NOT be shorter than 1 characters" zurück (alte Interfaces tragen sie, neu setzen geht nur ohne).
  1. **`shouldWaitForExecutionEnd` gehört in den `mapper` des Aufrufmoduls, nicht in `parameters`.** Der Make-Designer legt es dort ab; in `parameters` bleibt es wirkungslos. Alle Bestands-Aufrufe im Team tragen es in `parameters` — das fällt nur deshalb nicht auf, weil sie alle `false` wollen und das ohnehin der Default ist.
  2. **Das Aufrufmodul braucht die Kind-Outputs in seinem `metadata.interface`.** Make füllt das beim **Speichern im Designer** aus dem Kind (inklusive der `help`-Texte des Kind-Interfaces). Über die API von Hand geschrieben reicht allein nicht — beim Nachbauen den vom Designer erzeugten Block **verbatim** übernehmen.
  3. **Das konsumierende Modul direkt hinter den Aufruf setzen.** Mit einem `builtin:BasicRouter` dazwischen blieb die Rückgabe leer und die Laufzeit zeigte, dass gar nicht gewartet wurde (6,4 s statt 8,7–10,4 s). Ursache nicht endgültig isoliert — die verifizierte Form ist „Konsument direkt dahinter".
- **Modul-Ausgaben sind über den MCP nicht lesbar** (`executions_get-detail` liefert nur den Status). Ersatz-Messtechnik: ein `code:ExecuteCode` hinter dem Aufruf, das bei leerem Wert **wirft** — der Ausführungsstatus wird zum Messgerät. Für Mehrfachfragen Sonden hinter Filtern und die **Operations-Zahl** vergleichen: `code:ExecuteCode` zählt 1, `Start`/`Router`/`Merge`/`IfElse`/`CallSubscenario` zählen 0.
